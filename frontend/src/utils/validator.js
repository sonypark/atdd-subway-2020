const validator = {
  path: {
    source: [(v) => !!v || '출발역 입력이 필요합니다.', (v) => v >= 0 || '출발역 아이디는 0 이상의 양수이어야 합니다.'],
    target: [(v) => !!v || '도착역 입력이 필요합니다.', (v) => v >= 0 || '도착 아이디는 0 이상의 양수이어야 합니다.']
  },
  departureTime: {
    dayTime: [(v) => !!v || '오전 오후 입력이 필요합니다.', (v) => v in ['오전','오후'] || '입력값은 오전 또는 오후여야 합니다.'],
    hour: [(v) => !!v || '시간 입력이 필요합니다.', (v) => v >= 0 && v <= 12 || '숫자는 1~12 사이의 정수여야 합니다.' ],
    minute: [(v) => !!v || '분 입력이 필요합니다.',(v) => v >= 0 && v <= 60 || '숫자는 0~60 사이의 정수여야 합니다.']
  },
